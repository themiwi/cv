...
\textsl{Head of Turbine Digitalization (Linz)} \hfill \textbf{Januar 2019 -- Heute}\\
\vspace{-4mm}%
\begin{list2}
  \item Leitung eines Teams aus sieben Entwicklern, wovon zwei an anderen
    Standorten arbeiten. Erweiterung des Teams von vier auf sieben Entwickler
    durch Rekrutierung neuer Mitarbeiter in Österreich, Vietnam und Brasilien.
    Verantwortlich für die Fertigstellung im Zeit- und Budgetrahmen und für
  ...
  \item Entwicklung und Wartung verschiedener Softwarepakete, die von der F\&E,
    ...
    Diese sind entweder in der .NET Umgebung mit C\#, WinForms, MS~SQL~Server,
    ASP.NET~WebApi oder mit C++, MariaDB und Qt. Python wird oft für ad-hoc
    Programme, Datenanalyse und zum Verbinden grösserer Programme verwendet.
    Git mit Bitbucket und Azure DevOps, Jira, Confluence und Continuous
    Integration mit Jenkins werden intensiv zur Umsetzung von agilem
    Projektmanagement, zur Sicherstellung hoher Qualitätsstandards und zur
    Erstellung der Projektdokumentation genutzt.
  \item Umsetzung einer Datenbank für die parametrische CAD-Konstruktion
    ...
    und Erstellung von Schnittstellen für Berechnungswerkzeuge und Schaffung
    digitaler Datenströme. Dieses System ist mit .NET~Core WebApi, Angular,
    Blazor und Azure ActiveDirectory / OpenID Connect umgesetzt. Die
    Integration mit bestehenden Diensten wird mittels eines API Gateway
    erstellt.
  \item Enge Einbindung mehrerer bestehender Lösungen, Erstellen von digitalen
    Datenflüssen um die manuelle Datenerfassung zu reduzieren. Diverse
    Technologien, unter anderen C++, WebApi und Python kommen zum Einsatz.
\end{list2}

\textsl{Entwicklungsingenieur CAE Werkzeuge (Linz)} \hfill \textbf{August 2016 -- Dezember 2018}\\
\vspace{-4mm}%
\begin{list2}
  \item Agile Master und Systemarchitekt für die Industrial IoT Lösung METRIS
    ...
    auf der ANDRITZ METRIS Plattform. Diese besteht aus einem Kern der mit
    MS~SQL~Server und den Analysis Services, Entity~Framework, InfluxDB, WebApi
    und SignalR gebaut wurde. Analysemodule sind hauptsächlich mit Python,
    Pandas und scikit-learn umgesetzt und werden als Docker Container
    bereitgestellt. Ansible wird zur Provisionierung von Resourcen und die
    Bereitstellung in der Azure Cloud genutzt.
  ...
  \item Anwendung von Machine Learning und statistischer Analyse für die
    ...
    Domänenwissens, Entwicklung von Prototyp-Algorithmen. Vor allem Python,
    Jupyter Notebooks und Pandas werden hier genutzt, und wenn zweckmässig,
    wurden Power~Query und Power~BI eingesetzt.
\end{list2}

\pagebreak
\textsl{Entwicklungsingenieur CAE Werkzeuge (Zürich und Vevey)} \hfill \textbf{Mai 2013 -- Dezember 2018}\\
\vspace{-4mm}%
\begin{list2}
  ...
  \item Unter anderem Entwickler und Projektleiter von internen
    ...
    Microservices. Die Programme wurden vornehmlich mit C\#, WPF, WinForms,
    WebApi, MS~SQL~Server und Entity~Framework erstellt.
\end{list2}

\textbf{Basler Kantonalbank}\\\vspace{1mm}%
\textsl{Financial Engineer (Basel)} \hfill \textbf{Januar 2013 -- April 2013}\\
\vspace{-4mm}%
\begin{list2}
  \item Entwicklung und Wartung der Programme zur Verarbeitung und
    ...
    Kernbankensoftware. Eine Vielzahl an Technologien (C, Perl, Python, IBM MQ,
    Sybase, Oracle DB) und Betriebssystemen (Windows, HP UX, IBM AIX, Solaris,
    RHEL) wurde eingesetzt.
\end{list2}

\textbf{Institut für Fluiddynamik}, ETH Zürich\\\vspace{1mm}%
\textsl{Wissenschaftlicher Mitarbeiter (Zürich)} \hfill \textbf{Juni 2006 -- Dezember 2012}\\
\vspace{-4mm}%
\begin{list2}
  \item Entwickler eines MPI-parallelen Lösers mittels einem hybriden Ansatz aus
    ...
    Bibliothek OpenFOAM. Der C++ Simulationscode wurde auf Linux entwickelt.
    Python und ParaView wurden zur Datenverarbeitung und Visualisierung
    genutzt.
